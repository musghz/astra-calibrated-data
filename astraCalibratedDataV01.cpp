/// @file astraCalibratedDataV01.cpp
///
/// @brief Connect to Orbbec astra, get color and point streams,
/// align the point stream data and record a snapshot Stanford
/// .ply format with xyz rgb data.
///
...
int main(int argc, char** arvg)
{
	...
	while (!listener.is_finished() && !listener2.is_finished() && charCheckForKey != 27)
	{
		...
		flip(imgColor, imgColorMirrored, 1); // correct for mirror image
		flip(imgDepthFakeColor, imgDepthMirrored,1); // correct for mirror image
		flip(imgDepth, imgDepthRawMirrored,1); // correct for mirror image

		depthCorrectionCustom(imgDepthMirrored, imgDepthMirroredCorrected); // custom
...
void depthCorrectionCustom (Mat src, Mat dst) {
	...
	for(i=0;i<DWIDTH*DHEIGHT;i++) {
		...
		//iMirror = i + DWIDTH - i%DWIDTH;
		//currDepth = (int16_t)DEPTHZ[iMirror];
		if(currDepth>500 && currDepth < 2000) {

			//imgDepth.at<int16_t>(i/DWIDTH,i%DWIDTH) = 16*(int)(allFrameData[i].z);
			iMirror = i + DWIDTH - i%DWIDTH; // z data is mirrored, other is mirror corrected
