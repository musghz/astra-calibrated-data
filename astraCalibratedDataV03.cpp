/// @file astraCalibratedDataV03.cpp
///
...
int main(int argc, char** arvg)
{
	...
	while (!listener.is_finished() && !listener2.is_finished() && charCheckForKey != 27)
	{
		...
		generateInverseUVMap();
		syntheticDepthImage(imgDepthMirroredCorrected);
		...
		if(iterationCounter%30 == 0) {
			...
			//printf("idx[%d] at(%d,%d) u=%d, v=%d, isMapped=%d\n",DEPTHPT[1]*DWIDTH+DEPTHPT[0],DEPTHPT[0], DEPTHPT[1],UVMAPINV[DEPTHPT[1]*DWIDTH+DEPTHPT[0]].u,UVMAPINV[DEPTHPT[1]*DWIDTH+DEPTHPT[0]].v,UVMAPINV[DEPTHPT[1]*DWIDTH+DEPTHPT[0]].isMapped);
...
int savePLYFile() {
	printf("starting...\n");
	int i=0, r=0, g=100, b=200 ;
	float x, y, z, h, rad=10, theta;
	int iDepth=0, counter=0;
	Vec3b intensity;

	// file write stuff
	FILE * fp;
	fp = fopen ("ghaziABC.ply","w");
	if(fp==NULL){
	...
	// header info
	fprintf(fp,"ply\nformat ascii 1.0\ncomment author: Mustafa Ghazi\ncomment object: cloud data\nelement vertex %d\nproperty float x\nproperty float y\nproperty float z\nproperty uchar red\nproperty uchar green\nproperty uchar blue\nend_header\n",counter);
	...
	for(h=0;h<10;h+=1.0){
		...
		for(theta=0.0;theta<=6.28;theta+=0.02){
			...
			x = rad*cos(theta);
			y = rad*sin(theta);
			z += 1.0/(6.28/0.02); // n of z steps same as angle steps in 1 revolution
	...
	// replace the correct vertex count and delete this line
